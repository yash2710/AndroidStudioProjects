...
public final class PDF417
{
    ...
    private BarcodeMatrix b;
    private boolean c;
    private Compaction d;
    private int e;
    private int f;
    private int g;
    private int h;
    ...
    private static void a(int i, int j, a a1)
    {
        int k = 1 << j - 1;
        boolean flag;
        int l;
    ...
    private void a(CharSequence charsequence, int i, int j, int k, BarcodeMatrix barcodematrix)
    {
        int l = 0;
        int i1 = 0;
        while (i1 < j) 
        {
            ...
            if (j1 == 1)
            {
                k1 = 30 * (i1 / 3) + k * 3 + (j - 1) % 3;
                l1 = 30 * (i1 / 3) + (j - 1) / 3;
            } else
            {
                k1 = 30 * (i1 / 3) + (i - 1);
                l1 = 30 * (i1 / 3) + k * 3 + (j - 1) % 3;
            }
            a(a[j1][k1], 17, barcodematrix.b());
            for (int i2 = 0; i2 < i; i2++)
            {
                a(a[j1][charsequence.charAt(l)], 17, barcodematrix.b());
                l++;
            }

            if (c)
            {
                a(0x3fa29, 1, barcodematrix.b());
            } else
            {
                a(a[j1][l1], 17, barcodematrix.b());
                a(0x3fa29, 18, barcodematrix.b());
            }
            i1++;
        }
    }

    public final void generateBarcodeLogic(String s, int i)
    {
        int j = com.google.zxing.pdf417.encoder.b.a(i);
        String s1 = com.google.zxing.pdf417.encoder.c.a(s, d);
        int k = s1.length();
        float f1 = 0.0F;
        int ai[] = null;
        for (int l = e; l <= f; l++)
        {
            ...
            float f2 = (0.357F * (float)(69 + l * 17)) / (2.0F * (float)k2);
        ...
        if (1 + (k + j) > 929)
        {
            throw new WriterException((new StringBuilder("Encoded message contains to many code words, message to big (")).append(s.length()).append(" bytes)").toString());
        }
        int i2 = 1 + (k + l1);
        ...
        String s3 = com.google.zxing.pdf417.encoder.b.a(s2, i);
        String s4 = (new StringBuilder()).append(s2).append(s3).toString();
        b = new BarcodeMatrix(j1, i1);
        a(s4, i1, j1, i, b);
    }

    public final BarcodeMatrix getBarcodeMatrix()
    {
        return b;
    }
    ...
    public final void setCompaction(Compaction compaction)
    {
        d = compaction;
    }

    public final void setDimensions(int i, int j, int k, int l)
    {
        f = i;
        e = j;
        g = k;
