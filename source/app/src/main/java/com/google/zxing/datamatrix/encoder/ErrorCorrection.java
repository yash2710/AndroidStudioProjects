...
_L2:
        ...
        while (j1 < k + 0) 
        {
            int k1 = ac[i - 1] ^ charsequence.charAt(j1);
            int l1 = i - 1;
            while (l1 > 0) 
            {
                if (k1 != 0 && ai[l1] != 0)
                {
                    ac[l1] = (char)(ac[l1 - 1] ^ d[(c[k1] + c[ai[l1]]) % 255]);
                } else
                {
                    ac[l1] = ac[l1 - 1];
                }
...
_L3:
    ...
    {
        if (s.length() != symbolinfo.a)
        {
            throw new IllegalArgumentException("The number of codewords does not match the selected symbol");
        }
        StringBuilder stringbuilder = new StringBuilder(symbolinfo.a + symbolinfo.b);
        stringbuilder.append(s);
        int i = symbolinfo.getInterleavedBlockCount();
        if (i == 1)
        {
            stringbuilder.append(a(s, symbolinfo.b));
        } else
        {
            ...
            while (k < i) 
            {
                ...
                for (int l = k; l < symbolinfo.a; l += i)
                {
                    stringbuilder1.append(s.charAt(l));
                }

                String s1 = a(stringbuilder1.toString(), ai1[k]);
                int i1 = k;
                int l1;
                for (int j1 = 0; i1 < i * ai1[k]; j1 = l1)
                {
                    int k1 = i1 + symbolinfo.a;
                    l1 = j1 + 1;
                    stringbuilder.setCharAt(k1, s1.charAt(j1));
                    i1 += i;
                }
