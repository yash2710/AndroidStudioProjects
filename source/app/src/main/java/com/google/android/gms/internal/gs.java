...
public class gs
{
    ...
    private static final String CP = (new StringBuilder("yyyyMMdd'T'HHmmss")).append(CO[0]).toString();
...
_L6:
        ...
        if (s1.length() != 6 + CO[0].length()) goto _L7; else goto _L8
_L8:
    ...
    {
        int i = s.length();
        return i == 6 + CO[1].length() || i == 6 + CO[2].length() || i == 6 + CO[3].length();
    }
