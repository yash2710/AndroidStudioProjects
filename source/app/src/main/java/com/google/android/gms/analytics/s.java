...
_L2:
        ...
        _cls3.ux[ui.ordinal()];
        JVM INSTR tableswitch 1 6: default 341
    //                   1 104
    //                   2 201
    //                   3 341
    //                   4 341
...
_L4:
        d d2;
        for (; !um.isEmpty(); tU.a(d2.cR(), d2.cS(), d2.getPath(), d2.cT()))
        {
            d2 = (d)um.poll();
            aa.C((new StringBuilder("Sending hit to store  ")).append(d2).toString());
        }
...
_L9:
        d d1;
        d1 = (d)um.peek();
        aa.C((new StringBuilder("Sending hit to service   ")).append(d1).toString());
        if (ul.isDryRunEnabled()) goto _L12; else goto _L11
_L11:
        uj.a(d1.cR(), d1.cS(), d1.getPath(), d1.cT());
_L13:
        um.poll();
          goto _L5
_L12:
...
_L1:
        aa.D((new StringBuilder("Service unavailable (code=")).append(k).append("), will retry.").toString());
        cQ();
_L4:
        this;
        JVM INSTR monitorexit ;
        return;
_L2:
        aa.D((new StringBuilder("Service unavailable (code=")).append(k).append("), using local store.").toString());
        cO();
