...
public final class cc
{
    ...
    public static boolean a(Context context, ce ce1, cl cl1)
    {
        ...
        if (!TextUtils.isEmpty(ce1.oc))
        {
            ...
            if (as.length < 2)
            {
                eu.D((new StringBuilder("Could not parse component name from open GMSG: ")).append(ce1.oc).toString());
                return false;
            }
        ...
        try
        {
            eu.C((new StringBuilder("Launching an intent: ")).append(intent).toString());
            context.startActivity(intent);
            cl1.Y();
        }
        catch (ActivityNotFoundException activitynotfoundexception)
        {
            eu.D(activitynotfoundexception.getMessage());
            return false;
        }
        return true;
    }
