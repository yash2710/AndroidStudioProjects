...
public final class dy
{
    ...
    public static du a(Context context, ds ds1, String s)
    {
        ...
        try
        {
            JVM INSTR new #24  <Class JSONObject>;
        ...
        {
            eu.D((new StringBuilder("Could not parse the mediation config: ")).append(jsonexception.getMessage()).toString());
            return new du(0);
        }
        l = -1L;
...
_L2:
        ...
        if (eu.p(2))
        {
            JSONObject jsonobject1 = eo.o(hashmap);
            String s2;
            if (!(jsonobject1 instanceof JSONObject))
            {
                s2 = jsonobject1.toString(2);
            } else
            {
                s2 = JSONObjectInstrumentation.toString((JSONObject)jsonobject1, 2);
            }
            eu.C((new StringBuilder("Ad Request JSON: ")).append(s2).toString());
        }
        jsonobject = eo.o(hashmap);
        if (!(jsonobject instanceof JSONObject))
        {
            return jsonobject.toString();
        }
        String s1 = JSONObjectInstrumentation.toString((JSONObject)jsonobject);
        return s1;
        JSONException jsonexception;
        jsonexception;
        eu.D((new StringBuilder("Problem serializing ad request to JSON: ")).append(jsonexception.getMessage()).toString());
        return null;
