...
_L2:
        ...
        aeO = j;
        aey = l;
        w(Math.max(0L, Math.min(0x2932e00L, (0x2932e00L + aey) - aef.currentTimeMillis())));
        container = new Container(mContext, aeF.getDataLayer(), aet, l, j);
        if (aeM != null)
        {
            break; /* Loop/switch isn't completed */
        }
        aeM = new n(aeF, DF, container, aeI);
_L5:
        if (!isReady() && aeR.b(container))
        {
            b(aeM);
        }
...
_L3:
    ...
    public void lr()
    {
        ...
        if (c1 != null)
        {
            Container container = new Container(mContext, aeF.getDataLayer(), aet, 0L, c1);
            b(new n(aeF, DF, container, new _cls1()));
        } else
        {
...
label0:
            ...
            bh.C((new StringBuilder("setting refresh time to current time: ")).append(o.d(aeS)).toString());
            if (!o.e(aeS))
            {
                o.a(aeS, j);
            }
...
_L1:
            if (0x2932e00L + container.getLastRefreshTime() < o.a(aeS).currentTimeMillis()) goto _L4; else goto _L3
_L3:
            return true;
_L4:
            return false;
