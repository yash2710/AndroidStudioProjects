...
public class Tracker
{
    ...
    Tracker(String s, TrackerHandler trackerhandler, h h1, ae ae1, g g1, ad ad1, Context context)
    {
        ...
        xi = ae1;
        xj = g1;
        xf.put("&a", Integer.toString(1 + (new Random()).nextInt(0x7fffffff)));
        xg = ad1;
        xl = new a();
    ...
    void a(aj aj1)
    {
        ...
        if (xm.dE())
        {
            String s1 = xm.dF();
            set("&tid", s1);
            aa.C((new StringBuilder("[Tracker] trackingId loaded: ")).append(s1).toString());
        }
        if (xm.dG())
        {
            String s = Double.toString(xm.dH());
            set("&sf", s);
            aa.C((new StringBuilder("[Tracker] sample frequency loaded: ")).append(s).toString());
        }
        if (xm.dI())
        {
            setSessionTimeout(xm.getSessionTimeout());
            aa.C((new StringBuilder("[Tracker] session timeout loaded: ")).append(dy()).toString());
        }
        if (xm.dJ())
        {
            enableAutoActivityTracking(xm.dK());
            aa.C((new StringBuilder("[Tracker] auto activity tracking loaded: ")).append(dz()).toString());
        }
        if (xm.dL())
        {
            if (xm.dM())
    ...
    public void send(Map map)
    {
        ...
        {
            int i = 1 + Integer.parseInt((String)xf.get("&a"));
            if (i >= 0x7fffffff)
            {
                i = 1;
            }
        ...
        if (!s1.equals("transaction") && !s1.equals("item") && !xg._mthdo())
        {
